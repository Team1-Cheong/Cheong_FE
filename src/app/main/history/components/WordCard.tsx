...
export default function WordCard({
  title,
  tone,
  meaning = "뜻 설명 : ~~ 라는 뜻입니다. ~~ 라는 뜻입니다. ~~ 라는 뜻입니다",
  sentence = "작성된 예문",

  feedbackTitle = "한줄평",
  feedbackText = "아주 잘하셨어요! ~~라는 표현은 ~~이럴 때 쓰인답니다.",
  situationsTitle = "적용 상황",
  situations = ["~~할 때", "~~할 때", "~~할 때"],
}: Props) {
    ...
    >
      ...
      <button
        ...
        className={[
          ...
          "px-7 pb-5 pt-5", // ✅ (기존 px-8 pb-7 pt-7)
          "outline-none",
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <span
            className={[
              "mt-0.5 grid h-8 w-8 place-items-center rounded-xl", // ✅ 작게
      ...
      {/* ✅ 드롭다운 패널: padding/텍스트 전부 다운 */}
      <div
        id={panelId}
        className={[
          "relative px-7 pb-6 pt-5", // ✅ (기존 px-8 pb-8 pt-6)
          "transition-[max-height,opacity] duration-300 ease-out",
          open ? "max-h-[360px] opacity-100" : "max-h-0 opacity-0", // ✅ 살짝 축소
          "overflow-hidden",
        ].join(" ")}
      >
